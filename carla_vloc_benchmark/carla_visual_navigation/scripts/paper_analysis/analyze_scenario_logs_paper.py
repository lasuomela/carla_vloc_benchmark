...
def main():
    ...
    for town in experiment_files_grouped.keys():
        ...
        for main_category in experiment_files_grouped[town].keys():
            ...
            if 'autopilot' in experiment_files_grouped[town][main_category].keys():
                ...
                # Create plots
                create_failurerate_recall_plots(by_method_autopilot, by_method, odometry_failure_rate, odometry_fragmentation, plot_savedir, town_plotnames[town], exclude_ylabel=exclude_ylabel, exclude_legend=exclude_legend, plot_label=plot_label)
            ...
            for method in experiment_files_grouped[town][main_category].keys():
                for sub_category in experiment_files_grouped[town][main_category][method].keys(): 
                    ...
                    # Failure rates
                    #create_latex_string(by_method, '/dummy/weather.json', save_dir,  False, True)
                    # Recall rates
                   # create_latex_string(by_method_autopilot, '/dummy/weather.json', save_dir,  True, False)
    ...
    # save_dir = os.path.join(result_dir, 'town10', 'weather', experiment_files_grouped['town10']['weather']['experiment']['filename'].replace('.json', ''))
    # plot_savedir = os.path.join(save_dir, 'plots')
    # Path(plot_savedir).mkdir(parents=True, exist_ok=True)
    ...
    # # Create plots
    # create_failurerate_recall_plots(weather_by_method_autopilot, weather_by_method, odometry_failure_rate, odometry_fragmentation, plot_savedir, 'Town10', exclude_legend=False, plot_label='v')
    # #plot_crash_locations(aggregate, plot_savedir)
    # savepath=os.path.join(plot_savedir, 'delay_plot.png')
            ...
            for method in experiment_files_grouped[town][main_category].keys():
                ...
                if main_category == 'viewpoint':
                    ...
                    for kvalue in by_method.keys():
                        ...
                        create_failurerate_recall_plots(by_method_autopilot[kvalue], by_method[kvalue], odometry_failure_rate, odometry_fragmentation, plot_savedir, town_plotnames[town], exclude_ylabel=True, exclude_legend=True, plot_label=plot_label)
                        savepath=os.path.join(plot_savedir, 'delay_plot.png')
