...
def read_experiments(experiment_path, town, autopilot, odometry, exact_name):
    ...
    for scenario in combined_scenarios:
        ...
        if len(splitted_scenario)  <= 3:
            object_configs.append(f'/opt/carla_vloc_benchmark/src/carla_visual_navigation/config/objects_noview.json')
        elif len(splitted_scenario) > 3:
            obj = splitted_scenario[2]
            for object in object_configs_temp:
                if obj in object:
                    config_method = object.split('.')[0]
                    config_method = config_method.split('_')[1]
                    config_method = ''.join(i for i in config_method if not i.isdigit())
                    config_path=f'/opt/carla_vloc_benchmark/src/carla_visual_navigation/config/viewpoint_experiment_objects/{config_method}/{object}'
                    if os.path.isfile(config_path):
                        object_configs.append(f'/opt/carla_vloc_benchmark/src/carla_visual_navigation/config/viewpoint_experiment_objects/{config_method}/{object}')
                    else:
...
class ExperimentRunner(Node):
    ...
    def check_remaining_experiments(self):
        ...
        if os.path.isfile(log_file_path):
            ...
            print(execution_count, remaining_count)
    ...
    def scenario_finished_callback(self, scenario_status_msg):
        ...
        if scenario_status == 0:
            ...
            if not self.check_remaining_experiments():
                ...
                if len(self.experiments) <= self.last_idx+1:
